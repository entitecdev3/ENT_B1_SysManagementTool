sap.ui.define([
  ...
  return BaseController.extend("nvid.sample.controller.ManageMails", {
    ...
    handleSavePress : function () {
      var that = this;
      var payload = this.getView().getModel("local").getProperty("/config/hana-db-connection");
      payload.dedicated_config_override=true;
      dbAPI.callMiddleWare("/hanaDBConfig", "POST", payload).then(function(oData) {
    ...
    onTestPress: function(oEvent){
      ...
      dbAPI.callMiddleWare("/testHanaDB", "GET").then(function(oData) {
    ...
    onEditTestPress: function(oEvent){
      ...
      that.testBtn = oEvent.getSource();
      that.getView().setBusy(true);
      dbAPI.callMiddleWare("/validateHanaDBConfig", "POST",payload).then(function(oData) {
        that.testBtn.setIcon("sap-icon://message-success").setType("Accept").setText("Valid!");
        that.getView().setBusy(false);
        MessageToast.show("Synced")
      }).catch(function(oError) {
        that.testBtn.setIcon("sap-icon://message-error").setType("Reject").setText("Invalid!");
        dbAPI.errorHandler(oError, that);
      });
      setTimeout(function(){
        that.testBtn.setIcon("sap-icon://synchronize").setType("Emphasized").setText("Test");
